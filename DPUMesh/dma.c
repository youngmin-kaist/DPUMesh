					  ...
					  union doca_data ctx_user_data)
{
    ...
    result = put_free_dma_task(objs, dma_task);
    if (result != DOCA_SUCCESS)
				      ...
				      union doca_data ctx_user_data)
{
    ...
    int i;
    for (i = 0; i < objs->num_dma_tasks; i++) {
        entry = &objs->dma_task_entries[i];
...
init_dma_tasks(struct objects *objs, int num_tasks)
{
    ...
    result = doca_buf_inventory_start(objs->buf_inv);
    if (result != DOCA_SUCCESS) {
    ...
    result = doca_dma_create(objs->dev, &objs->dma_ctx);
    if (result != DOCA_SUCCESS) {
    ...
    result = doca_dma_cap_get_max_num_tasks(objs->dma_ctx, &max_tasks);
    if (result != DOCA_SUCCESS) {
    ...
    objs->num_dma_tasks = num_tasks;
    DOCA_LOG_INFO("DOCA DMA max number of tasks: %u", max_tasks);

    dma_ctx = doca_dma_as_ctx(objs->dma_ctx);
    ...
    result = doca_dma_task_memcpy_set_conf(objs->dma_ctx,
    ...
    objs->dma_task_entries = calloc(num_tasks, sizeof(*objs->dma_task_entries));
    if (objs->dma_task_entries == NULL) {
        result = DOCA_ERROR_NO_MEMORY;
        goto destroy_dma;
    }

    TAILQ_INIT(&objs->free_dma_tasks);
    TAILQ_INIT(&objs->submission_dma_tasks);
    objs->num_free_dma_tasks = 0;
    objs->num_submission_dma_tasks = 0;
    for (i = 0; i < num_tasks; ++i) {
        struct dma_task_entry *entry = &objs->dma_task_entries[i];

        result = doca_dma_task_memcpy_alloc_init(objs->dma_ctx,
        ...
        entry->owner = objs;
        entry->result = DOCA_SUCCESS;
        result = put_free_dma_task(objs, entry->task);
        if (result != DOCA_SUCCESS) {
            doca_task_free(doca_dma_task_memcpy_as_task(entry->task));
            entry->task = NULL;
            goto free_tasks;
...
free_tasks:
    while (i-- > 0)
        doca_task_free(doca_dma_task_memcpy_as_task(objs->dma_task_entries[i].task));
    free(objs->dma_task_entries);
    objs->dma_task_entries = NULL;
    objs->num_dma_tasks = 0;
    objs->num_free_dma_tasks = 0;
    objs->num_submission_dma_tasks = 0;
destroy_dma:
...
submit_dma_task(struct objects *objs, const struct doca_buf *src, struct doca_buf *dst)
{
    ...
    if (objs == NULL || src == NULL || dst == NULL)
        return DOCA_ERROR_INVALID_VALUE;

    dma_task = get_free_dma_task(objs);
    if (dma_task == NULL)
    ...
    if (result == DOCA_SUCCESS) {
        ...
        entry->result = result;
        if (put_free_dma_task(objs, dma_task) != DOCA_SUCCESS)
...
enqueue_dma_task(struct objects *objs, const struct doca_buf *src, struct doca_buf *dst)
{
    ...
    result = put_submission_dma_task(objs, dma_task);
    if (result != DOCA_SUCCESS && put_free_dma_task(objs, dma_task) != DOCA_SUCCESS)
...
progress_dma_submission_queue(struct objects *objs, int max_tasks, int *num_submitted)
{
    ...
    if (objs == NULL || max_tasks < 0)
        return DOCA_ERROR_INVALID_VALUE;

    while (max_tasks == 0 || submitted < max_tasks) {
        dma_task = get_submission_dma_task(objs);
        if (dma_task == NULL)
            break;
        ...
        if (result == DOCA_ERROR_AGAIN) {
            TAILQ_INSERT_HEAD(&objs->submission_dma_tasks, entry, entries);
            entry->in_submission_queue = true;
            objs->num_submission_dma_tasks++;
        } else if (put_free_dma_task(objs, dma_task) != DOCA_SUCCESS) {
...
get_free_dma_task(struct objects *objs)
{
    ...
    entry = TAILQ_FIRST(&objs->free_dma_tasks);
    if (entry == NULL)
        return NULL;

    TAILQ_REMOVE(&objs->free_dma_tasks, entry, entries);
    entry->in_free_queue = false;
    objs->num_free_dma_tasks--;
...
put_free_dma_task(struct objects *objs, struct doca_dma_task_memcpy *dma_task)
{
    ...
    if (objs == NULL || dma_task == NULL)
        return DOCA_ERROR_INVALID_VALUE;

    entry = doca_task_get_user_data(doca_dma_task_memcpy_as_task(dma_task)).ptr;
    if (entry == NULL || entry->owner != objs || entry->task != dma_task ||
    ...
    TAILQ_INSERT_TAIL(&objs->free_dma_tasks, entry, entries);
...
get_submission_dma_task(struct objects *objs)
{
    ...
    entry = TAILQ_FIRST(&objs->submission_dma_tasks);
    if (entry == NULL)
        return NULL;

    TAILQ_REMOVE(&objs->submission_dma_tasks, entry, entries);
...
put_submission_dma_task(struct objects *objs, struct doca_dma_task_memcpy *dma_task)
{
    ...
    TAILQ_INSERT_TAIL(&objs->submission_dma_tasks, entry, entries);
    entry->in_submission_queue = true;
    objs->num_submission_dma_tasks++;
...
{
    return objs == NULL ? 0 : objs->num_free_dma_tasks;
}

int
get_num_submission_dma_tasks(const struct objects *objs)
{
    return objs == NULL ? 0 : objs->num_submission_dma_tasks;
}
...
cleanup_dma_tasks(struct objects *objs)
{
    ...
    if (objs == NULL || objs->dma_ctx == NULL)
        return;
    ...
    if (objs->dma_task_entries != NULL) {
        for (i = 0; i < objs->num_dma_tasks; ++i) {
            if (objs->dma_task_entries[i].task != NULL)
                doca_task_free(doca_dma_task_memcpy_as_task(objs->dma_task_entries[i].task));
        }
    }
